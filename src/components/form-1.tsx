...
export function Form2({ searchParams: { utm_campaign, utm_content, utm_medium, utm_source, utm_term, pagina } }: FormProps) {
    ...
    useEffect(() => {
        ...
        if (cont === 0) {
            ...
            script.src = 'https://duasporuma.activehosted.com/f/embed.php?id=34';
    ...
    useEffect(() => {
        ...
        const submitButton = document.getElementById('_form_34_submit') as HTMLButtonElement;
        if (submitButton) {
    ...
    return (
        <div className="relative w-full h-[256px]">
            ...
            <div id="_forms_34" className={`_form_34 max-w-sm -translate-y-4 z-50`}></div>
        </div>
    );
}
